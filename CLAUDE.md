...
- `App.jsx` — phone shell + 4-tab bottom nav. `Wallpaper` (sky/ribbon/grain/glow decorative layers), `StatusBar`, `AppHeader` (live/closed chip), `BottomNav` (floating pill nav with a measured sliding indicator), `NavLensFilter` (lens-displacement SVG filter feeding the indicator's live refraction, see Bottom nav below), `AppShell` (tab useState + `tabMotionKey` for active-state animation). Shell `.wrap` carries `id="phone-shell"`. Default export wraps in `<AssetProvider>`.
...
**Sliding indicator** (`.nav-indicator`): `transform: translateX(...)` + `width`, computed from `getBoundingClientRect()` of the active button. As of 2026-07-06 the motion is **GSAP-driven** (`back.out(1.7)`, 0.55s — the Aave-style spring settle the user asked for; overshoot is now intentional), NOT a CSS transition — `.nav-indicator`/`.nav-lens-copy` deliberately have no transform/width transitions in CSS, or they'd fight the tweens. The pill tween and the copy's counter-translate tween share one duration/ease so they cancel mid-flight. `prefersReducedMotion()` snaps with `gsap.set` instead.
...
**Liquid Glass refraction** (wired up 2026-07-06, Aave technique — aave.com/design/building-glass-for-the-web): `NavLensFilter` in `App.jsx` generates a cubic-power displacement map into a canvas → SVG `feImage` once on mount. The filter is applied as a plain `filter: url(#nav-lq)` (cross-browser) on `.nav-lens` inside `.nav-indicator` — **not** `backdrop-filter: url()`, which is Chromium-only. The lens refracts a white copy of the tab row (`.nav-lens-copy` — white, not gold: the user explicitly asked for the gold label to be removed, so the refraction shows with no color tint; counter-translated in a GSAP tween sharing the pill's spring so the two transforms cancel and the copy stays glued to the real buttons; the indicator sits at `z-index: 3`, above the buttons, so the refracted copy is what reads as "selected"). The filter is Aave's full chain: **three** `feDisplacementMap` passes at staggered scales (30/36/42), each isolated to one color channel via `feColorMatrix`, recomposed with additive `feComposite arithmetic` — that scale stagger IS the chromatic fringe that makes the refraction visible; a single pass at scale ~24 reads as nothing. Hard-won details, don't regress: (1) `colorInterpolationFilters` must be the literal `sRGB` — `sRGBLinear` is invalid, silently falls back to `linearRGB`, and shifts the whole refraction down-right ~7px; (2) `feImage` needs explicit `width`/`height` set to the pill's box (done in `BottomNav`'s measure) — without them it renders the map at natural size and the uncovered area displaces by −scale/2 (same down-right ghost), and Firefox needs the explicit size regardless; (3) the map is horizontal-only (G channel = neutral 128) because the label sits flush against the pill's bottom edge and any Y-bend ghosts the text below itself; (4) `feTurbulence` = WRONG (shaky noise); filter region stays `y="-28%" height="156%"`. Safari untested as of 2026-07-06 (no macOS box) — if the lens freezes mid-motion there, that's Safari's filter-ID output cache; give the filter a fresh id per map update.
