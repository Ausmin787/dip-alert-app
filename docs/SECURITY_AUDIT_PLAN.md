...
| 2026-07-21 | Local release-candidate audit | Remediated delivery-state consumption without configured credentials, historical amount drift, one-time default seeding, migration coverage, scheduler/global-market mismatch, endpoint validation and bounds, credential clearing, test-alert concurrency/rate limiting, query-string log exposure, Yahoo request amplification, mode-inaccurate UI copy, inaccessible dialog controls, polling overlap, and vulnerable frontend transitive dependencies | Logic, security, migration, deploy-safety, compile, dependency, frontend test/lint/build, and production-style runtime checks rerun on the final tree before publish | Source-level gaps fixed; SEC-001..004 and the SEC-005 live-process confirmation remain infrastructure-only | Commit/push the verified release candidate, then complete the separately authorized VM/infra checks |
